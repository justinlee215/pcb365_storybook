...
export default function EditUSCustomsInvoice({ USCustomsInvoice }) {
  ...
  const {
    ...
    defaultValues: USCustomsInvoice,
  });
  ...
  const onSubmit = async (content) => {
    ...
    const response = await fetch(
      `http://localhost:3000/api/forms/USCustomsInvoice/${USCustomsInvoice._id}`,
      {
        ...
        body: JSON.stringify({ USCustomsInvoice: content }),
      }
    )
      .then((res) => {
        return res.json();
  ...
  const ShipperFields = () => (
    <section>
      ...
      <Input name="formType" type="hidden" value="US Customs Invoice" />
      <Input
        label="NAME"
        name="shipperName"
