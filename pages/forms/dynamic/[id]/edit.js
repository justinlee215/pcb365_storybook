...
export default function Editdynamic({ dynamic }) {
  console.log("Dynamic edit page: ", dynamic);
  ...
  const onSubmit = async (content) => {
    ...
    const response = await fetch(
      `http://localhost:3000/api/forms/dynamic/${dynamic._id}`,
      {
        ...
        body: JSON.stringify({ dynamic: content }),
      }
    )
      .then((res) => {
        return res.json();
